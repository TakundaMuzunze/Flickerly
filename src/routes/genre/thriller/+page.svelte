<script lang="ts">
	...
	onMount(() => {
		window.addEventListener('scroll', () => handleScroll('53', sortBy, currentPage, totalPages, isLoading));
		return () => {
			window.removeEventListener('scroll', () => handleScroll('53', sortBy, currentPage, totalPages, isLoading));
		};
	});
</script>

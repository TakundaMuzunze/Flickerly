<script lang="ts">
	...
	onMount(async () => {
		await fetchMovies('10749', 'popularity', 1);
		document.title = 'Romance Movies | Flickerly';
	});

	// Add scroll event listener
	onMount(() => {
		window.addEventListener('scroll', () => handleScroll('10749', sortBy, currentPage, totalPages, isLoading));
		return () => {
			window.removeEventListener('scroll', () => handleScroll('10749', sortBy, currentPage, totalPages, isLoading));
		};
	});
</script>
...
>
	<div class="space-y-2">
		<h2 class="relative text-xl font-semibold text-white md:text-2xl">Romance Movies</h2>
		<p class="text-gray-300 md:text-lg">{genreSubtitles.romance}</p>
		<FilterBar />
	</div>

<script lang="ts">
	...
	onMount(async () => {
		await fetchMovies('14', 'popularity', 1);
		document.title = 'Fantasy Movies | Flickerly';
	});

	// Add scroll event listener
	onMount(() => {
		window.addEventListener('scroll', () => handleScroll('14', sortBy, currentPage, totalPages, isLoading));
		return () => {
			window.removeEventListener('scroll', () => handleScroll('14', sortBy, currentPage, totalPages, isLoading));
		};
	});
</script>
...
>
	<div class="space-y-2">
		<h2 class="relative text-xl font-semibold text-white md:text-2xl">Fantasy Movies</h2>
		<p class="text-gray-300 md:text-lg">{genreSubtitles.fantasy}</p>
		<FilterBar />
	</div>

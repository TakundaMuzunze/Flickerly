<script lang="ts">
	...
	onMount(async () => {
		await fetchMovies('878', 'popularity', 1);
		document.title = 'Science Fiction Movies | Flickerly';
	});

	// Add scroll event listener
	onMount(() => {
		window.addEventListener('scroll', () => handleScroll('878', sortBy, currentPage, totalPages, isLoading));
		return () => {
			window.removeEventListener('scroll', () => handleScroll('878', sortBy, currentPage, totalPages, isLoading));
		};
	});
</script>
...
>
	<div class="space-y-2">
		<h2 class="relative text-xl font-semibold text-white md:text-2xl">Science Fiction Movies</h2>
		<p class="text-gray-300 md:text-lg">{genreSubtitles.scienceFiction}</p>
		<FilterBar />
	</div>

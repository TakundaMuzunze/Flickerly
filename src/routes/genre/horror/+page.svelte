<script lang="ts">
	...
	onMount(async () => {
		await fetchMovies('27', 'popularity', 1);
		document.title = 'Horror Movies | Flickerly';
	});

	// Add scroll event listener
	onMount(() => {
		window.addEventListener('scroll', () => handleScroll('27', sortBy, currentPage, totalPages, isLoading));
		return () => {
			window.removeEventListener('scroll', () => handleScroll('27', sortBy, currentPage, totalPages, isLoading));
		};
	});
</script>
...
>
	<div class="space-y-2">
		<h2 class="relative text-xl font-semibold text-white md:text-2xl">Horror Movies</h2>
		<p class="text-gray-300 md:text-lg">{genreSubtitles.horror}</p>
		<FilterBar />
	</div>
